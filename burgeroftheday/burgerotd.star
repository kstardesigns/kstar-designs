...
# get time zone
# get time now and day now
# check if current year has Feb 29 (is divisible by 4 but not by 100). if not, remove [59] (day 60, feb 29) from each array
# get day difference from today since Jan 1 to get what day number
# list burger and parenthetical like BURGER_LIST[23]

def main(config):
    TIMEZONE = config.get("$tz", "America/Phoenix")  # Utilize special timezone variable
    DEFAULT_TIME = time.now().in_location(TIMEZONE).format("2006-01-02T15:04:05Z07:00")
    FIRST_DAY = time.parse_time("2022-01-01T15:04:05Z")
    DAY_NUMBER = time.now().in_location(TIMEZONE) - FIRST_DAY

    # # # # LEFT OFF:
    # # # # need to convert DAY_NUMBER to how many days have passed, then to an integer that can be used to show  in array
    ...
    # if BURGER_SHOWN == "daily":
    #     BURGER_NAME = BURGER_LIST[DAY_NUMBER]
    #     BURGER_PAR = BURGER_PAR_LIST[DAY_NUMBER]
    # if BURGER_SHOWN == "custom":
    #     BURGER_NAME = BURGER_LIST[DAY_NUMBER]
    #     BURGER_PAR = BURGER_LIST[DAY_NUMBER]
    else:
        BURGER_NAME = "not random"
        BURGER_PAR = "not random"
